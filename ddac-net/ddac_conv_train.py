...
class ActorNetwork(object):
    ...
    def create_actor_network(self): 
        inputs = tflearn.input_data(shape=[None, 12, 12, 1])
        net = tflearn.conv_2d(inputs, 8, 5, activation='relu', name='actor_conv1')

        net = tflearn.fully_connected(net, 100, activation='relu')
        net = tflearn.fully_connected(net, 50, activation='relu')
        # Final layer weights are init to Uniform[-3e-3, 3e-3]
        w_init = tflearn.initializations.uniform(minval=-0.003, maxval=0.003)
        action_scores = tflearn.fully_connected(net, self.a_dim, activation='tanh', weights_init=w_init)
        probs = tflearn.activations.softmax (action_scores)
        # scaled_action_scores = tf.mul(action_scores, self.action_bound) # Scale action_scoresput to -action_bound to action_bound
        return inputs, action_scores, probs
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        inputs = tflearn.input_data(shape=[None, 12, 12, 1])
        net = tflearn.conv_2d(inputs, 8, 5, activation='relu', name='critic_conv1')
...
def train(sess, env, actor, critic):
    ...
    for i in xrange(MAX_EPISODES):
        ...
        for j in xrange(MAX_EP_STEPS):
            ...
            a = actor.predict(np.reshape(s, (1, 12, 12, 1)))
            probs = a[0]
            # action_score = a[0]
            ...
            replay_buffer.add(np.reshape(s, (12, 12, 1)), np.reshape(a, (actor.a_dim,)), r, \
                terminal, np.reshape(s2, (12, 12, 1)))
            ...
            if terminal:
                ...
                if i%EVAL_EPISODES == 0 and i!=0 :
                    eval_acc_reward_summary = eval_acc_reward
                    print(' 100 round reward: ', eval_acc_reward_summary)
                    eval_acc_reward = 0
