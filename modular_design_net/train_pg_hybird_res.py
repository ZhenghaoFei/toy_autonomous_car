...
SUMMARY_DIR = './results_pg_hybird_res/'
RANDOM_SEED = 1234
...
class ActorNetwork(object):
    ...
    """
    def __init__(self, sess, state_dim, action_dim, learning_rate, layers):
        self.sess = sess
    ...
    def create_actor_network(self): 
        ...
        for i in range(self.layers - 1):
            ...
            net = tflearn.activation(tf.matmul(fc1,fc2.W) + tf.matmul(features_res, fc_res.W) + fc2.b + fc_res.b, activation='relu')
        ...
        net = tf.add(tf.multiply(net_plain, alpha), tf.multiply(net_res, (1-alpha)))
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        actor = ActorNetwork(sess, state_dim, action_dim, ACTOR_LEARNING_RATE, LAYERS)
