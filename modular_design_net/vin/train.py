...
tf.app.flags.DEFINE_integer('ch_h1',          150,                    'Channels in initial hidden layer')
tf.app.flags.DEFINE_integer('ch_h2',          150,                    'Channels in initial hidden layer')
...
if (config.untied_weights):
  ...
  # nn = PI_block(X, S1, S2, config)
  nn = Plain_Block(X, S1, S2, config)

# Define loss and optimizer
dim = tf.shape(y)[0]
cost_idx = tf.concat([tf.reshape(tf.range(dim), [dim,1]), tf.reshape(y, [dim,1])], 1)
cost = -tf.reduce_mean(tf.gather_nd(tf.log(tf.clip_by_value(nn, 1e-10, 1.0)), [cost_idx]))
  ...
  for epoch in range(int(config.epochs)):
    ...
    for i in range(0, Xtrain.shape[0], batch_size):
      ...
      if j <= Xtrain.shape[0]:
        ...
        # print('i', i, 'j', j)
        # print('y range: ', i * config.statebatchsize, j * config.statebatchsize)
        nn_eval = sess.run(nn, feed_dict=fd)
        # print(np.asarray(nn_eval)).shape
