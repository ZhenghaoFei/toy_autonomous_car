...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        # indices = tf.range(0, tf.shape(self.log_prob)[0]) * tf.shape(self.log_prob)[1] + self.actions_in
            ...
            if terminal:
                ...
                if i%EVAL_EPISODES == 0:
                    ...
                    summary_str = sess.run(summary_ops, feed_dict={
                        summary_vars[0]: (eval_acc_reward/EVAL_EPISODES),
                    })
                    writer.add_summary(summary_str, i)
                    writer.flush()

                    print ('| score: %i ' % (eval_acc_reward/EVAL_EPISODES), "| Episode", i, \
...
def main(_):
    ...
    monitor_dir = SUMMARY_DIR
    # env.monitor.start(monitor_dir, force=True)
    print("s_dim:", env.observation_space.shape)
    state_dim = [80, 80, 1]
